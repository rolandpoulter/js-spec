...
module.exports = Options.clss('Spec', function (def, supr, proto) {
	...
	def.callBeforeTest = function (scope, callback, testHook) {
		return this.enumParents(function (parent, next) {
			...
			this.callHookFunc(testHook, 'testBefore', scope, function (error) {
				if (error) return callback.call(this, error);
	...
	def.addSpec = function (name, options, block) {
		var spec = new Spec(name, options, block).setParent(this)

		if (!this.isFrozen) this.specs[name] = spec;
				...
				this.callBeforeTest(scope, function (error) {
					...
					function end (error) {
						...
						that.callAfterTest(lastScope, function (error) {
							...
							that.callHookFunc(unit.options.last, 'last', scope, function (error) {
	...
	function describe (name, options, block) {
		return context.addSpec(name, options, block);
	}
	...
	function _describe (name, options, block) {
		return context.focusSpec(context.addSpec(name, options, block))
	}
